...
async function ensureSystemAccount() {
  ...
  let systemUser = await userModel.findOne({ email: systemEmail });
...
async function applyWelcomeBonus({ user, targetAccount }) {
...
async function verifyOtpController(req, res) {
  try {
    ...
    // Apply welcome bonus (runs outside any session so ledger writes succeed)
    let bonus = null;
    try {
      bonus = await applyWelcomeBonus({ user, targetAccount: account });
    } catch (error) {
